...
test('simple-ns', function (t) {
  
  //second arg=true means stream of raw js objects,
  //do not stringify/parse.
  var a = gelrpc.stream(obj, true);
  b = gelrpc.stream(null, true);
...
test('tcp-ns', function (t) {
  var net = require('net');
  var port = Math.round(40000 * Math.random()) + 1000;
  var a = gelrpc.stream(obj);
  var b = gelrpc.stream();
...
test('child_process-ns', function (t) {
  var cp = require('child_process')
  .spawn(process.execPath, [require.resolve('../stream/support/cp')]);
  var b = gelrpc.stream();
